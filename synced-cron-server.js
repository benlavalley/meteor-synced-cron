...
Meteor.startup(function () {
	...
	if (options.customDB) {
		SyncedCron._collection = new Mongo.Collection(options.collectionName, { _driver: options.customDB });
	} else {
	...
	if (options.additionalIndex) {
		let createIndexes;
		if (!_.isArray(options.additionalIndex)) {
			createIndexes = [options.additionalIndex];
		} else {
			createIndexes = options.additionalIndex;
		}
...
SyncedCron._laterSetTimeout = function (fn, sched) {
	const s = Later.schedule(sched); let
		t;
	scheduleTimeout();
  ...
  */
	function scheduleTimeout() {
		...
		let intendedAt = next[0] && new Date(next[0].setMilliseconds(0, 0)); // added code to round off millseconds / round down to nearest second. This attempts to fix an issue with simultaneous jobs running because their milliseconds were slightly different. May not need but leaving here anyway as the below function appears to try and ensure the same thing. Method comes from https://stackoverflow.com/questions/3830244/get-current-date-time-in-seconds
